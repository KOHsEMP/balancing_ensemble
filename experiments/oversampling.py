...
parser = argparse.ArgumentParser(description='Experiment to compare performance of proposed methods with oversampling and without oversampling.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
parser.add_argument('--minority_rate', required=True, type=float)
...
class Config:
    ...
    eval_data_size = 40000 # 評価用データのサンプルサイズ
...
def sampling_data(cfg, df_data, show=False):
    ...
    for i in range(cfg.n_models):
        ...
        df_tmp = sampled_dataset.iloc[i*cfg.each_model_data_size : (i+1)*cfg.each_model_data_size] # 上から順に
...
cfg.minority_rate = args.minority_rate
...
def experiment_info(cfg):
    ...
    exp_info += f'minority_rate:{cfg.minority_rate}'
...
for exp in range(cfg.num_experiment):
    ...
    ensemble_labeled_size = len(df_ensemble_labeled_train)

    ## oversampling
    from imblearn.over_sampling import SMOTE
    if len(cfg.labeled_classes) ==2:
        sm = SMOTE(sampling_strategy=cfg.minority_rate, random_state=cfg.seed) # 'not majority'
        X = df_ensemble_labeled_train.drop('target', axis=1).values
        Y = df_ensemble_labeled_train['target'].values
        columns_name = df_ensemble_labeled_train.drop('target', axis=1).columns

        X_res, Y_res = sm.fit_resample(X, Y)
        X_res = pd.DataFrame(X_res, columns=columns_name)
        Y_res = pd.DataFrame(Y_res).rename(columns={0:"target"})

        df_ensemble_labeled_train_resampled = pd.concat([X_res, Y_res], axis=1)
    else: # if multiclass
        majority_class_size = 0 # 一番サンプルが多いクラスのサンプルサイズ
        value_counts_list = df_ensemble_labeled_train['target'].value_counts() # クラスごとのサンプルサイズのリスト
        # majority_class_sizeの決定
        for c in range(10):
            if value_counts_list[c] > majority_class_size:
                majority_class_size = value_counts_list[c]

        minority_classes = [] # cfg.minority_rateに基づき，oversamplingが必要と判断されたクラスのリスト
        # minority_classesの洗い出し
        for c in range(10): 
            if value_counts_list[c] < majority_class_size * cfg.minority_rate:
                minority_classes.append(c)
        # oversampling
        sm = SMOTE(sampling_strategy='not majority', random_state=cfg.seed)
        ...
        df_data_res = pd.concat([X_res, Y_res], axis=1)

        orig_size = len(df_ensemble_labeled_train) # oversampling前のサンプルサイズ
        over_sampled_df = pd.DataFrame() # SMOTEでoversamplingしたデータから，必要な分だけ抽出したものを入れる
        for c in minority_classes:
            sample_size = int(majority_class_size * cfg.minority_rate - value_counts_list[c]) # oversamplingが必要と判定される閾値まで，足りてない分だけ抽出する
            tmp_df = df_data_res.iloc[orig_size:][df_data_res['target']==c].sample(n=sample_size, random_state=cfg.seed)
            over_sampled_df = pd.concat([over_sampled_df, tmp_df], axis=0)
        
        df_ensemble_labeled_train_resampled = pd.concat([df_ensemble_labeled_train, over_sampled_df], axis=0)

    
    ## component models: training and inferring
    pred_ensemble_train_labels, pred_ensemble_train_probs, pred_eval_labels, pred_eval_probs = make_models(cfg, datasets, df_ensemble_labeled_train_resampled, df_eval)
    ...
    # extract supervised data part 
    ## prediction probabilities of component models for this supervised data
    labeled_train_probs = [] # prediction probability for original data
    labeled_train_probs_resampled = [] # prediction probability for data including oversampled data
    for i in range(cfg.n_models):
        labeled_train_prob = pred_ensemble_train_probs[i][:ensemble_labeled_size].copy()
        labeled_train_probs.append(labeled_train_prob)
        labeled_train_prob_resampled = pred_ensemble_train_probs[i].copy()
        labeled_train_probs_resampled.append(labeled_train_prob_resampled)
    ...
    # Proposed supervised ensemble learning (+ Oversampling) ====================================================================
    ## decide lambda by cross validation # use original data & oversamled data
    best_lam = tuning_proposed_supervised(cfg, labeled_train_probs_resampled, df_ensemble_labeled_train_resampled, criterion='accuracy_score',how=cfg.tuning, show_res=False)
    ## weight parameter optimization & inferring # use only original data
    tuned_beta, eval_exp_mix_prob = proposed_supervised_ensemble(cfg, labeled_train_probs, df_ensemble_labeled_train, pred_eval_probs, best_lam, show=False)
    ## evaluation
    scores = evaluation(cfg, df_eval, eval_exp_mix_prob)
    experiment_results_proposed_oversampling.append(scores)
    experiment_best_lams_oversampling.append(best_lam)
    tuned_results['oversampling_beta'].append(tuned_beta)
    tuned_results['oversampling_lambda'].append(best_lam)
    tuned_results['oversampling_scores'].append(scores)
    print(scores)
...
save_result_csv_oversampling(cfg, experiment_results_component, 
                                experiment_results_unsupervised,
                                experiment_results_supervised,
                                experiment_results_proposed_supervised,
                                experiment_results_proposed_oversampling,
...
show_result_oversampling(cfg, experiment_results_component, 
                            experiment_results_unsupervised,
                            experiment_results_supervised,
                            experiment_results_proposed_supervised,
                            experiment_results_proposed_oversampling,
...
content = f"best_lams (oversampling): {experiment_best_lams_oversampling}\n"
with open(cfg.EXP_LOG + f"/log_{cfg.MODEL_NAME}.txt", "a") as appender:
    appender.write(content + "\n")
