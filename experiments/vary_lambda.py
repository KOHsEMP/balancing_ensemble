...
parser = argparse.ArgumentParser(description='Experiment to investigate lambda and accuracy for test data.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
class Config:
    ...
    algorithm_list_list = [["logistic", "logistic", "svm", "svm", "knn"],
                            ["logistic", "random_forest", "svm", "knn", "gaussian_naive_bayes"],
                            ["random_forest", "random_forest", "knn", "knn", "logistic"],
                            ["random_forest", "random_forest", "logistic", "svm", "knn"],
...
def experiment_info(cfg):
    ...
    exp_info += f"algorithms:{cfg.algorithm_list_list}\n"
...
experiment_results_per_component["logistic"] = []
experiment_results_per_component['svm'] = []
experiment_results_per_component['knn'] = []
experiment_results_per_component['decision_tree'] = []
experiment_results_per_component['QDA'] = []
experiment_results_per_component['gaussian_process'] = []
experiment_results_per_component['gaussian_naive_bayes'] = []
experiment_results_per_component['adaboost'] = []
experiment_results_per_component['random_forest'] = []

lambda_list = np.linspace(0, 1.0, cfg.num_tuning+1).tolist()
eval_list = np.zeros((len(cfg.algorithm_list_list), len(lambda_list)))

for exp in range(cfg.num_experiment):
    for alg_list_idx, algorithm_list in enumerate(cfg.algorithm_list_list):
        print(f"start alg_list_idx : {alg_list_idx}")
        cfg.algorithm_list = algorithm_list # update algorithm_list
        ...
        for i in range(cfg.n_models):
            ...
            experiment_results_per_component[algorithm_list[i]].append(scores) # scores of each component model
        ...
        for lam_idx, lam in enumerate(lambda_list):
            ...
            eval_list[alg_list_idx][lam_idx] += scores['accuracy_score']
...
vline_min = eval_list.min()
vline_max = eval_list.max()
ax.vlines(0.5, vline_min, vline_max+0.01, 'gray', linestyles='dashed')

for alg_list_idx in range(len(cfg.algorithm_list_list)):
    ax.plot(lambda_list, eval_list[alg_list_idx],
            #marker="o",#_markers[alg_list_idx % len(_markers)],
            linestyle='solid',
            color=_colors[alg_list_idx % len(_colors)],
            label=f"{extracted_model_names_list[alg_list_idx]}",)
...
fig.savefig(os.path.join(cfg.MAIN_PATH, "output", cfg.NAME, "fig", f'{cfg.DATA_NAME}_exp{cfg.num_experiment}.pdf'), bbox_inches="tight", pad_inches=0.05)
fig.savefig(os.path.join(cfg.MAIN_PATH, "output", cfg.NAME, "fig", f'{cfg.DATA_NAME}_exp{cfg.num_experiment}.png'), bbox_inches="tight", pad_inches=0.05)

# save log
save_data = {}
save_data["alg_list_list"] = cfg.algorithm_list_list
save_data["lambda_list"] = lambda_list
save_data["eval_list"] = eval_list
save_data["cfg"] = cfg
save_data['component_score'] = experiment_results_per_component

pickle.dump(save_data, open(os.path.join(cfg.EXP_LOG, f"{cfg.DATA_NAME}_exp{cfg.num_experiment}_log.pkl"), 'wb'))
